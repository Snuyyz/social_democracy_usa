...
d3.linegraph = function(noTicks, noDots, parties, partyColors, partyNames, dataMax, dataMin, additionalMonths) {
    /* params */
    if (!parties) {
        parties = ['spd', 'kpd', 'ddp', 'z', 'dnvp', 'other'];
    }
    if (!partyColors) {
        partyColors = {'spd': '#2330ba', 'kpd': '#6b107d', 'ddp': '#1ab841', 'z': '#e0b30d', 'dnvp': '#e0140d', 'other': '#a0a0a0'};
    }
    if (!partyNames) {
        partyNames = {'spd': 'Democrats', 'kpd': 'PSL', 'ddp': 'Greens', 'z': 'Libertarian', 'dnvp': 'Republicans', 'other': 'Independents'};
    }
    if (!additionalMonths) {
        additionalMonths = 10;
    }
